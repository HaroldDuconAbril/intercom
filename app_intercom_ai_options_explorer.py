...
:root{
  ...
  --blue:#2563eb;--violet:#7c3aed;--green:#16a34a;--amber:#d97706;--red:#dc2626;
}
.block-container{padding-top:1rem!important;padding-left:2rem!important;padding-right:2rem!important;max-width:1440px;}
html,body,[class*="css"]{font-size:14.2px;color:var(--ink)}
h1{font-size:1.55rem!important;line-height:1.15!important;margin-bottom:.25rem!important} h2{font-size:1.16rem!important} h3{font-size:1.02rem!important;margin-top:.5rem!important}
.stMarkdown p,.stMarkdown li{font-size:.89rem!important;line-height:1.4!important}.small{font-size:.78rem;color:var(--muted)}
.hero{background:linear-gradient(135deg,#020617 0%,#1d4ed8 55%,#7c3aed 100%);padding:20px 24px;border-radius:20px;color:white;margin-bottom:15px;box-shadow:0 18px 42px rgba(15,23,42,.22)}
.hero h1{color:white!important;margin:0!important}.hero p{color:#dbeafe!important;margin:.45rem 0 0 0!important}.hero .kicker{font-size:.76rem;letter-spacing:.08em;text-transform:uppercase;color:#bfdbfe;font-weight:700}
.card{background:var(--card);border:1px solid var(--line);border-radius:17px;padding:15px 17px;box-shadow:0 4px 18px rgba(15,23,42,.045);margin-bottom:11px}.soft{background:#f8fafc}
.explain{background:#f8fafc;border-left:4px solid var(--blue);border-radius:13px;padding:11px 13px;margin:9px 0 13px;color:#334155;font-size:.86rem;line-height:1.42}.explain strong{color:#0f172a}.warn{border-left-color:var(--amber);background:#fffbeb}.success{border-left-color:var(--green);background:#f0fdf4}.research{border-left-color:var(--violet);background:#faf5ff}
...
SOLUTIONS = [
    {
        ...
        "req":["Plan Intercom o Fin standalone","Knowledge Hub/FAQs organizado","Reglas de handoff","Agentes/equipos configurados","Medición de outcomes mensuales"],
        "rubros":[
            ("Fin AI Agent","USD 0,99/outcome aprox.","Un outcome es un resultado exitoso medible. En atención al cliente suele significar que Fin resolvió una conversación o completó un procedimiento configurado sin que el agente tenga que intervenir de forma manual. No es lo mismo que un mensaje: una conversación puede tener varios mensajes, pero normalmente se cobra una vez si el resultado se considera exitoso."),
            ("Intercom seat","Desde USD 29/seat/mes aprox.","Es el costo de usuarios o agentes que trabajan dentro del helpdesk de Intercom. Este rubro depende del tamaño del equipo humano, no del número de preguntas de clientes."),
            ("Configuración inicial","USD 0 - 500","Incluye cargar contenido en Knowledge Hub, ajustar tono, definir reglas de escalamiento, probar respuestas y validar qué temas deben ir siempre a humano.")],
    ...
    },
    {
        ...
        "rubros":[
            ("Desarrollo inicial","USD 1.500 - 8.000","Construcción a la medida: backend, conexión con Intercom, reglas de negocio, clasificación de preguntas, lógica de alta demanda, handoff a agentes, pruebas y despliegue. Es el rubro más alto porque se crea una solución propia."),
            ("Hosting/API","USD 20 - 200/mes","Servidor o servicio cloud que mantiene el bot disponible. Debe recibir webhooks de Intercom en tiempo real y responder sin interrupciones."),
            ("OpenAI/Azure OpenAI","Según uso","Se paga por consumo del modelo IA. Depende del volumen de conversaciones, longitud de preguntas, contexto enviado y longitud de respuestas."),
            ("Mantenimiento","USD 100 - 500/mes","Ajustes de prompts, mejora de respuestas, revisión de errores, actualización de FAQs y monitoreo de casos escalados.")],
...
with right:
    if selected:
        ...
        with tab1:
            ...
            if s.get("fin"):
                st.info(f"Referencia usada para comparar: {int(CONVERSACIONES_MES*PCT_IA/100)} outcomes potenciales al mes + 3 seats. Si Fin resuelve menos conversaciones, baja el variable; si resuelve más, sube. La lectura correcta no es solo costo, sino costo versus ahorro de tiempo operativo.", icon="ℹ️")
            else:
                st.info(f"Referencia usada para comparar: {int(CONVERSACIONES_MES*PCT_IA/100)} conversaciones procesadas por IA. El costo del modelo puede ser bajo frente a desarrollo/licencias, pero puede crecer con volumen, contexto documental y complejidad de respuestas.", icon="ℹ️")
        with tab2:
            for r in s["req"]:
                st.write(f"- {r}")
